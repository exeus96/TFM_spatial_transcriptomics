...
kDatasetNames <- c("MB12", "MB14", "MB15", "MB22", "MB7", "MB9", "MB21", "MB50", "MB52") %>% 
  setNames(., .)
...
samplegroups <- list(
  control = c('MB7', 'MB9', 'MB15', 'MB21'),
  schizo = c('MB12', 'MB14', 'MB22', 'MB50', 'MB52')
)
...
cao <- Cacoa$new(con_spat, 
                 ref.level="Ctr", 
                 target.level="Scz", 
                 sample.groups=setNames(c(rep("Ctr", 4),rep("Scz", 5)),unlist(samplegroups, use.names = FALSE)), 
                 cell.groups=(annot_spat), 
                 n.cores=5)
de_spat_annot_mujeres <- cao$estimatePerCellTypeDE(n.cores = 20)
save(de_spat_annot_mujeres,file = './data/de_spat_annot_mujeres.Rdata')
```

```{r}

for (element in de_spat_annot_mujeres) {
  print(element$res[element$res$padj < 0.05,])
...
calculateGosGSEA <- function(de, go.datas, n.top.genes=500,n.cores=1) {
  de <- de[unlist(lapply(de,is.list))]
  # add Z scores
  de <- lapply(de,function(d) {
    res.table <- d$res;
    print(res.table)
    res.table$Z <- res.table$pval
    res.table$Za <- -qnorm(res.table$padj/2)
    res.table$Za[is.na(res.table$Za)] <- 0
    d$res <- res.table;
    d
  })
    gns <- list(down=lapply(de,function(x) x$res[order(x$res$Z,decreasing=F),]),
              up=lapply(de,function(x) x$res[order(x$res$Z,decreasing=T),]),
              all=lapply(de,function(x) x$res))
...
GSEAGOOpt <- function (gene, OrgDB, goData, keyType = "ENTREZID", ont = "MF", pvalueCutoff = 0.05,
                         pAdjustMethod = "BH", universe=NULL, qvalueCutoff = 0.2, minGSSize = 10,
                         maxGSSize = 500, readable = FALSE, pool = FALSE) {
  ont %<>% toupper %>% match.arg(c("BP", "CC", "MF"))

  nombres = rownames(gene)
  gene = gene[,"pvalue"]
  names(gene) = nombres
  ...
  names(gene) = new_names
  gene <- unlist(gene[!is.na(names(gene))])
  gene <-sort(gene, decreasing = TRUE)

  res <- clusterProfiler:::gseGO(geneList = gene,
                                 ont = "BP",
                                 pAdjustMethod = 'BH',
                                 OrgDb = OrgDB,
                                 nPermSimple = 100000,
...
gos_spat_gsea <- list(
  annot=calculateGosGSEA(de_spat_annot_mujeres, go_datas, n.top.genes=500)
)

gos_spat <- list(
  annot=calculateGos(de_spat_annot_mujeres, go_datas, n.top.genes=500)
)
...
gos.up.gsea <- mapply(function(x,y) {
    GSEAGOOpt(gene = x, universe = y, ont = "BP", goData = go_datas[['BP']], 
              OrgDB = org.Hs.eg.db::org.Hs.eg.db, readable = T)},
  x = gos_spat_gsea$annot[[c("all")]],
  y = gos_spat_gsea$annot[[c("all")]])
...
save(gdc_bp_summ_df,file = './data/guc_bp_summ_df_down_mujeres.Rdata')
save(cols,file = './data/cols_mujeres.Rdata')
...
save(guc_bp_summ_df,file = './data/guc_bp_summ_df_up_mujeres.Rdata')
...
for (layer in de_spat_annot_mujeres) {
...
save(reactome_per_layer,file = './data/reactome_per_layer_mujeres.Rdata')
