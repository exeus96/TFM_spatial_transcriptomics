...
samplegroups <- list(
  control_men = c('MB11', 'MB19', 'MB18-2'),
  schizo_men = c('MB6', 'MB23'),
  control_women = c('MB15','MB7','MB9','MB21'),
  schizo_women = c('MB12', 'MB14', 'MB22', 'MB50', 'MB52')
)
...
SpatialPlot(samples$"MB52"$seu,pt.size.factor = 4) + theme(legend.position = "right")
```
...
estimateDEPerCellTypeInner <- function(raw.mats, cell.groups=NULL, s.groups=NULL, ref.level=NULL, target.level=NULL,
  ...
  print('Justo antes')
  de.res <- names(cm.bulk.per.type) %>% sn()%>% lapply(function(l) {
     print('Justo despues')
    ...
    meta$control_men = as.factor(ifelse(meta$group == 'control_men',1,0))
    meta$schizo_men = as.factor(ifelse(meta$group == 'schizo_men',1,0))
    meta$control_women = as.factor(ifelse(meta$group == 'control_women',1,0))
    meta$schizo_women = as.factor(ifelse(meta$group == 'schizo_women',1,0))
    print('If Anterior')
    ## External covariates
    if (is.null(meta.info)) {
      #meta$sex <- relevel(as.factor(ifelse(meta$group == 'control_men' | meta$group == 'schizo_men' ,'men','women')),ref='men')
      #meta$group <- relevel(as.factor(ifelse(meta$group == 'control_men' | meta$group == 'control_women','control','schizo')),ref='control')
      print(meta)
      #design.formula <- as.formula('~ group + sex + group:sex')
      design.formula <- as.formula('~(schizo_men - control_men) - (schizo_women - control_women)')
      print(as.character(design.formula))
      print('Entra en 1')
    } else {
    ...
    if (test %in% c('wilcoxon', 't-test')) {
      ...
      print('Entra en DESeq2')
      print(test.type)
      res <- estimateDEForTypeDESeq(
        cm, meta=meta, design.formula=design.formula, ref.level='control', target.level='schizo',
    ...
    return(res)
    #, n.cores=n.cores, progress=FALSE, mc.preschedule=TRUE, mc.allow.recursive=TRUE
  })  %>%
    .[!sapply(., is.null)]
...
estimateDEForTypeDESeq <- function(cm, meta, design.formula, ref.level, target.level, test.type, ...) {
  ...
  res %<>% DESeq2::results(name="schizo_men_1_vs_0", ...) %>% as.data.frame()
...
#' @export Cacoa
Cacoa_mod <- R6::R6Class("Cacoa_mod", lock_objects=FALSE,
  public = list(
    ...
    initialize=function(data.object, sample.groups=NULL, cell.groups=NULL, sample.per.cell=NULL,
      ...
      if(is.null(sample.groups.palette)) {
        self$sample.groups.palette <- c("#d73027", "#4575b4","#d73027", "#4575b4") %>%
    ...
    estimateDEPerCellType=function(cell.groups=self$cell.groups, sample.groups=self$sample.groups,
      ...
      aux = list()
      #"control_men", 3),rep("schizo_men", 2),rep("control_women", 4),rep("schizo_women"
      aux[['control_men']] = c()
      aux[['schizo_men']] = c()
      aux[['control_women']] = c()
      aux[['"schizo_women']] = c()
      if (!is.list(sample.groups)) {
        for (i in 1:length(sample.groups)){
          if (sample.groups[i] == 'control_men'){
           aux[['control_men']] = append(aux[['control_men']],names(sample.groups[i]))
          }
          if (sample.groups[i] == 'schizo_men'){
           aux[['schizo_men']] = append(aux[['schizo_men']],names(sample.groups[i]))
          }
          if (sample.groups[i] == 'control_women'){
           aux[['control_women']] = append(aux[['control_women']],names(sample.groups[i]))
          }
          if (sample.groups[i] == 'schizo_women'){
           aux[['schizo_women']] = append(aux[['schizo_women']],names(sample.groups[i]))
          }
...
class(con_spat)
cao <- Cacoa_mod$new(con_spat,
                ref.level="control_men",
                target.level="schizo_men",
                sample.groups=setNames(c(rep("control_men", 3),rep("schizo_men", 2),rep("control_women", 4),rep("schizo_women", 5)),unlist(samplegroups, use.names = FALSE)), #
...
#                  cell.groups=(annot_spat), 
#                  n.cores=5)
de_spat_annot <- cao$estimateDEPerCellType(n.cores = 20)
```
...
for (layer in de_spat_annot) {
  ...
  gene_list = gene_list[gene_list$res.pvalue < 0.05,]
  ...
  #                pAdjustMethod = 'BH',
  #                pvalueCutoff = 0.05)
  kegg_per_layer <- append(kegg_per_layer,kk)
}

head(kegg_per_layer[2])
gos_spat <- list(
...
Heatmap(as.matrix(guc_bp_summ_df),
             col=cols$up,
              border=T,
              show_row_dend=F,
              show_column_dend=F, 
