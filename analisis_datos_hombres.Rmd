...
kDatasetNames <- c("MB11", "MB18-2", "MB23","MB6", "MB19") %>% 
  setNames(., .)
...
cao <- Cacoa$new(con_spat, 
                 ref.level="Ctr", 
                 target.level="Scz", 
                 sample.groups=setNames(c(rep("Ctr", 3),rep("Scz", 2)),unlist(samplegroups, use.names = FALSE)), 
                 cell.groups=(annot_spat), 
                 n.cores=5)
de_spat_annot_hombres <- cao$estimatePerCellTypeDE(n.cores = 20)
save(de_spat_annot_hombres,file = './data/de_spat_annot_hombres_hombres.Rdata')
```
...
gos_spat_gsea <- list(
  annot=calculateGosGSEA(de_spat_annot_hombres, go_datas, n.top.genes=500)
)

gos_spat <- list(
  annot=calculateGos(de_spat_annot_hombres, go_datas, n.top.genes=500)
)
...
save(gdc_bp_summ_df,file = './data/guc_bp_summ_df_down_hombres.Rdata')
save(cols,file = './data/cols_hombres.Rdata')
...
save(guc_bp_summ_df,file = './data/guc_bp_summ_df_up_hombres.Rdata')
...
for (layer in de_spat_annot_hombres) {
...
save(reactome_per_layer,file = './data/reactome_per_layer_hombres.Rdata')
...
save(go_gsea_per_layer,file='./data/go_gsea_per_layer_hombres.Rdata')
```
